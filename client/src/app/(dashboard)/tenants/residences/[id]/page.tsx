...
const Residence = () => {
    ...
    const { data: leases, isLoading: leaseLoading, error: leaseError } = useGetLeasesQuery(

      parseInt(authUser?.cognitoInfo?.userId || "0") || 0,
    ...
    const { data: payments, isLoading: paymentLoading, error: paymentError } = useGetPaymentsQuery(
      ...
      if(paymentLoading || leaseLoading || isPropertyLoading) return <Loading/>

      if(!property || propertyError) return <div>Error loading property</div>

      if(!leases || leaseError) return <div>Error loading leases</div>

      if(!payments || paymentError) return <div>Error loading payments</div>
